...
# ==============================================================================
# 一锤定音大模型工具-仅pt引擎 (CHUIZI)
# 版本：1.1 (PT-Only + FineTune Optional)
# 作者：锤子代码（公众号）
# 功能：交互式下载大模型，生成推理脚本和微调脚本（仅支持 PT 引擎，微调可选）
# ==============================================================================
...
show_welcome() {
 ...
 | (__  | __ | | |_| |  | |   / /   | |  | |) |  / _ \   | |  | |\/| |  / _ \ 
  \___| |_||_|  \___/  |___| /___| |___| |___/  /_/ \_\ |___| |_|  |_| /_/ \_\
       ...
       一锤定音大模型工具 (CHUIZI) - 版本 1.1 (微调可选)
       作者：锤子代码（公众号）
       代码仓库：https://github.com/jpg1024/yichuidingyin-big-model-tool
  
EOF
    echo -e "欢迎使用一锤定音大模型工具（仅支持 PT/transformers 引擎）！\n"
}
...
ask_finetune_option() {
    ...
    while true; do
        read -p "$(echo -e '是否需要微调模型，请输入y或者N? ')" finetune_choice
        ...
        if [[ "$finetune_choice" == "y" ]]; then
            ...
            echo -e "✅ 已选择：需要微调模型\n"
            break
        elif [[ "$finetune_choice" == "N" ]]; then
            NEED_FINETUNE="N"
            echo -e "✅ 已选择：跳过微调，仅生成推理脚本\n"
            break
        else
            echo -e "⚠️  输入无效！请严格输入小写 y 或大写 N"
            echo -e "   • 输入 y  → 进入微调参数配置"
            echo -e "   • 输入 N  → 跳过微调，直接生成推理脚本\n"
        fi
    done
}
...
generate_inference_scripts() {
    ...
    # 推理脚本文件名（仅保留 PT 引擎）
    INFERENCE_PT_SCRIPT="$SCRIPTS_DIR/1-命令行推理.sh"
    APP_PT_SCRIPT="$SCRIPTS_DIR/2-界面推理.sh"
    DEPLOY_PT_SCRIPT="$SCRIPTS_DIR/3-API接口部署.sh"
...
📦 已生成脚本列表：
├─ 推理类脚本:
│  ├─ $TARGET_DIR/scripts/1-命令行推理.sh
│  ├─ $TARGET_DIR/scripts/2-界面推理.sh
│  └─ $TARGET_DIR/scripts/3-API接口部署.sh
├─ 微调类脚本:
...
1️⃣ 命令行推理:
   $ chmod +x $TARGET_DIR/scripts/1-命令行推理.sh
   $ $TARGET_DIR/scripts/1-命令行推理.sh

2️⃣ Web界面启动:
   $ chmod +x $TARGET_DIR/scripts/2-界面推理.sh
   $ $TARGET_DIR/scripts/2-界面推理.sh
   # 访问: http://服务器IP:6006

3️⃣ API服务部署:
   $ chmod +x $TARGET_DIR/scripts/3-API接口部署.sh
   $ $TARGET_DIR/scripts/3-API接口部署.sh
   # 接口地址: http://服务器IP:6008/v1/chat/completions

4️⃣ 开始微调:
   $ chmod +x $TARGET_DIR/scripts/4-微调.sh
   $ $TARGET_DIR/scripts/4-微调.sh
   # 微调结果保存在: $FINETUNE_OUTPUT_DIR

5️⃣ 微调后推理/合并:
   $ # 编辑 5-微调后推理-需修改路径.sh 和 6-合并微调-需修改路径.sh
   $ # 将 /path/to/fine-tuned-lora 替换为实际的 checkpoint 路径

💡 提示:
   • 所有脚本已自动添加 --trust_remote_code true 参数
   • 如果网络较慢，脚本已配置使用 hf-mirror.com 镜像加速
   • 微调前建议先运行推理脚本，确认模型加载正常

感谢使用一锤定音大模型工具 (公众号：锤子代码)！
...
main() {
    ...
    # 根据用户选择决定是否生成微调脚本
    if [[ "$NEED_FINETUNE" == "y" ]]; then
        configure_finetune_params   # 配置微调参数
        generate_finetune_scripts   # 生成微调脚本
    else
        echo -e "⏭️  已跳过微调配置，仅保留推理脚本。\n"
    fi
    
    show_completion                 # 显示完成信息（动态内容）
}
